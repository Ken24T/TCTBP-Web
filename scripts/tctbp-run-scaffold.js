...
} = require("./tctbp-managed-surface");
const { generateProfile } = require("./tctbp-scaffold-profile");
...
async function main(cliOptions) {
  ...
  writeSourceMetadata(answers.targetPath);
...
function writeSourceMetadata(targetPath) {
  let policy;
  try {
    policy = JSON.parse(fs.readFileSync(path.join(GITHUB_DIR, "TCTBP.json"), "utf8"));
  } catch (error) {
    console.error(`Could not read source policy: ${error.message}`);
    return;
  }
  ...
  try {
    sourceRevision = spawnSync("git", ["rev-parse", "HEAD"], {
      cwd: SCAFFOLD_REPO_ROOT,
      encoding: "utf8"
    }).stdout.trim();
  } catch {
    // no git context available — leave revision empty
  }

  let sourceVersion = "";
  try {
    sourceVersion = JSON.parse(
      fs.readFileSync(path.join(SCAFFOLD_REPO_ROOT, "VERSION"), "utf8")
    ).version;
  } catch {
    // no VERSION file — leave version empty
  }
  ...
  const metadata = createSourceMetadata({
    ...
    adviserContract: policy.adviserContract
  });

  const targetTctbp = path.join(targetPath, ".tctbp");
  fs.mkdirSync(targetTctbp, { recursive: true });
  fs.writeFileSync(
    path.join(targetTctbp, "source.json"),
    JSON.stringify(metadata, null, 2) + "\n",
    "utf8"
  );
  console.log("Wrote .tctbp/source.json source metadata.");
}
